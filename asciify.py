...
def modify(image, buckets=25, blank_thres_hold=0.9):
    initial_pixels = list(image.getdata())
    pixels = []
    cnt = Counter()

    for pixel_value in initial_pixels:
        char = ASCII_CHARS[pixel_value // buckets]
        cnt[char] += 1

        pixels.append(char)

    # Replace the top of [thres_hold] with blank
    top_len = round(len(cnt) * (1 - blank_thres_hold))
    most_common_pixels = cnt.most_common(top_len)
    for tpx, _ in most_common_pixels:
        new_pixels = [x if x != tpx else " " for x in pixels]
        pixels = new_pixels
